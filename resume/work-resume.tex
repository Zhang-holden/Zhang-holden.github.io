...
\IfFontExistsTF{Times New Roman}{\setmainfont[AutoFakeBold=2.2]{Times New Roman}}{}
...
\else
    \IfFontExistsTF{KaiTi}{
        % Windows 中文楷体（正文、标题、粗体统一使用楷体；AutoFakeBold 模拟加粗，数值越小越不易糊）
        \setCJKmainfont[AutoFakeBold = 2.2, ItalicFont = KaiTi]{KaiTi}
...
% ── 页面布局 ──────────────────────────────────────────────
\usepackage{geometry}
\geometry{a4paper, left=1.3cm, right=1.3cm, top=0.5cm, bottom=0.5cm}
...
\newlength{\photow}
\settowidth{\photow}{\includegraphics[height=\profileh]{assets/profile-photo.png}}
\newlength{\oldphotow}
\settowidth{\oldphotow}{\includegraphics[height=\photoh]{assets/profile-photo.png}}
...
\vspace{\dimexpr 0.8em-6pt\relax}
...
\rinfo{\textbf{数据分析与工程应用}：能够将数字化工具与工程业务有效融合，掌握 XML、XQuery 等数据管理技术，能熟练运用 Python 进行工程数据分析与处理，在成本优化与碳资产管理等领域有研究经验。}

\rinfo{\textbf{基础设施规划与评估}：熟悉相关 ISO 国际标准，能熟练应用全生命周期评估（LCA）及灾害情景模拟方法，结合具体场景开展基础设施的气候适应性分析与韧性规划。}

\rinfo{\textbf{沟通协作与外语能力}：具备跨文化交流与对外展示能力，可将英语作为工作语言；拥有真实企业合作场景下的团队协作与成果汇报经验。}
...
\noindent
\textbf{再生面粉产品全生命周期评估}\hfill{}\textbf{硕士研究项目}

\rinfo{研究概述：作为研究团队成员，与新西兰环保企业 Rescued Kitchen 合作，基于全生命周期评估方法框架，使用 OpenLCA 对产品从原料到成品全过程的环境影响进行量化评估，识别主要环境影响并提出改进方向；研究成果向企业联合创始人汇报，并被奥克兰大学官网新闻及新西兰晨间新闻报道。}
...
\section*{技能资格}
\vspace{-0.1em}
\rinfo{\textbf{语言能力}：CET-6、奥克兰大学学术英语证书。}

\rinfo{\textbf{数据编程}：Python（Pandas、Matplotlib等）、智能合约（Solidity）、XML、XPath、XQuery、LaTex。}

\rinfo{\textbf{工程软件}：Revit、AutoCAD、OpenLCA。}

\rinfo{\textbf{其他}：二级篮球裁判员、新西兰 Level-1 篮球裁判员。}
